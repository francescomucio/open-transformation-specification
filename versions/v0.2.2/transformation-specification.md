# Open Transformation Specification v0.2.2
  ...
  - transformation_id: string    # Fully qualified identifier (e.g., "analytics.my_first_table")
    ...
    schema:
      columns:                   # Array of column definitions
        - name: string           # Column name
          ...
          auto_incremental: bool # Optional: If true, system automatically calculates stable IDs using MAX(id) + ROW_NUMBER() pattern
    ...
    materialization:
      ...
      incremental_details:       # Required if type is "incremental"
        ...
        filter_condition: string # SQL condition for filtering data (derived from filter_column + start_value if provided)
        filter_column: string   # Optional: Column name to use for time-based filtering (preferred over filter_condition for granular control)
        start_value: string      # Optional: Start value for incremental loading ("auto", "CURRENT_DATE", specific value, or variable reference)
        destination_filter_column: string # Optional: Column name in target table for MAX() subquery (if different from filter_column)
        lookback: string         # Optional: Lookback period for late-arriving data (e.g., "7 days", "3 hours")
        ...
        on_schema_change: string # (Optional) How to handle schema changes: "fail" (default), "ignore", "append_new_columns", "sync_all_columns", "full_refresh", "full_incremental_refresh", "recreate_empty"
      full_incremental_refresh:   # Optional: Configuration for full_incremental_refresh on_schema_change behavior
        parameters: [            # Array of parameter configurations for incremental chunking
          - name: string         # Parameter name (matches placeholder in query, e.g., "@start_date", "@end_date")
            start_value: string  # Initial value for the parameter
            end_value: string    # End condition: hardcoded value (e.g., "2025-12-31") or expression evaluated against source table (e.g., "max(event_date)" from source.events)
            step: string         # Increment step: SQL interval (e.g., "INTERVAL 1 DAY") or numeric value
...
When using incremental materialization, the output schema of a transformation may differ from the existing target table schema. The `on_schema_change` parameter controls how these differences are handled:

- **`fail`** (default): Fail the transformation if any schema differences are detected. This ensures explicit handling of schema changes and prevents unexpected behavior.

- **`ignore`**: Ignore schema differences and proceed with the incremental operation. The transformation will use the existing table schema, potentially causing errors if columns don't match. Use this when schema changes are handled manually or when you want explicit control over schema evolution.

- **`append_new_columns`**: Automatically add new columns from the transformation output to the target table. Existing columns remain unchanged. Missing columns in the output are not removed from the table.

- **`sync_all_columns`**: Synchronize the target table schema with the transformation output by adding new columns and removing missing columns. **Warning**: This may cause data loss when columns are removed.

- **`full_refresh`**: Drop the existing table and recreate it with the full transformation output (no incremental filtering). This performs a complete refresh of the table.

- **`full_incremental_refresh`**: Drop the existing table, recreate it, then immediately run the incremental strategy in chunks until reaching the end condition. Requires `full_incremental_refresh` configuration with parameter definitions.

- **`recreate_empty`**: Drop the existing table and recreate it as an empty table (no data). Useful when backfilling is done using a different tool.

**Type Mismatches**: If a column exists with the same name but different data type, it is treated as a schema change and handled according to the `on_schema_change` setting. With `fail` (default), this will cause the transformation to fail immediately.

**Column Order**: Column order differences are detected and logged as warnings. Tools should rely on explicit column lists in INSERT/MERGE statements to ensure correct data alignment.
...
materialization:
  ...
  incremental_details:
    ...
    on_schema_change: "fail"  # Optional: default is "fail"
```

#### Full Incremental Refresh Example

When `on_schema_change` is set to `"full_incremental_refresh"`, the transformation will drop and recreate the table, then run incremental chunks:
...
full_incremental_refresh:
  parameters:
    - name: "@start_date"
      ...
      end_value: "max(event_date)"  # Expression evaluated against source table, or hardcoded: "2025-12-31"
      step: "INTERVAL 1 DAY"
```

For queries with two parameters (boundaries):
...
full_incremental_refresh:
  parameters:
    - name: "@start_date"
      start_value: "2024-01-01"
      end_value: "max(event_date)"  # Expression evaluated against source table
      step: "INTERVAL 1 MONTH"
    - name: "@end_date"
      start_value: "2024-01-31"
      end_value: "max(event_date)"  # Expression evaluated against source table
      step: "INTERVAL 1 MONTH"
```

**Note**: When `end_value` is an expression (e.g., `"max(event_date)"`), it is evaluated against the **source table(s)** referenced in the transformation query, not the target table. This allows the incremental refresh to determine the end condition based on the actual data available in the source.
...
materialization:
  ...
  incremental_details:
    ...
    # Recommended (OTS 0.2.2+): use filter_column + start_value (+ optional destination_filter_column)
    filter_column: "updated_at"   # Column in the source query used for filtering
    start_value: "auto"           # "auto" uses MAX(...) from the target table at runtime
    # Optional: if the target table stores the timestamp under a different name, set destination_filter_column:
    # destination_filter_column: "updated_dt"
    update_columns: ["name", "email"]  # Optional: specific columns to update
```

**Example (column renamed between source and target):**

If your source query filters on `created_date` but your target table stores the processed watermark as `created_dt`,
you can set `destination_filter_column`:
...
materialization:
  ...
  incremental_details:
    strategy: "merge"
    merge_key: ["brand_name"]
    filter_column: "created_date"              # Source column
    destination_filter_column: "created_dt"    # Target column used for MAX()
    start_value: "auto"
```
